...
C     !LOCAL VARIABLES:
      ...
      Real*8 maskp1 ! mask
      Real*8 Kyy_S, Kyz_S ! Tensor elements at the southern boundary
      Real*8 epsS ! Avoiding zero denominator
      Real*8 dR_W_jk, dR_W_jm1k, dR_W_jkm1
      Real*8 dR_W_jm1km1, dR_W_k, dR_W_km1 ! Gradients at W grids, used for calculating dCdz_S
      Real*8 dCdz_S, dCdy_S ! Tracer gradients at the southern boundary / W grids
...
C-    Calculate the tensor element at the southern boundary
        Kyy_S = ( maskS(i,j  ,k,bi,bj)*slcon_Kyy(i,j  ,k,bi,bj)
     &          + maskS(i,j-1,k,bi,bj)*slcon_Kyy(i,j-1,k,bi,bj) ) /
     &          (maskS(i,j  ,k,bi,bj) + maskS(i,j-1,k,bi,bj) + epsS)
        Kyz_S = ( maskS(i,j  ,k,bi,bj)*slcon_Kyz(i,j  ,k,bi,bj)
     &          + maskS(i,j-1,k,bi,bj)*slcon_Kyz(i,j-1,k,bi,bj) ) /
     &          (maskS(i,j  ,k,bi,bj) + maskS(i,j-1,k,bi,bj) + epsS)
C-    Calculate the tracer gradients at the southern boundary
C     Calculate dC/dy at the southern boundary
        dCdy_S = (Tracer(i,j,k)-Tracer(i,j-1,k))*maskS(i,j,k,bi,bj)
     &          *recip_dyC(i,j,bi,bj)*recip_deepFacC(k)
C     Calculate dC/dz at W grids
        maskp1 = 1.D0
        IF (k.GE.Nr) maskp1 = 0.D0
        dR_W_jkm1   = ( Tracer(i,j  ,km1) - Tracer(i,j  ,k) )
     &               * recip_drC(k) 
        dR_W_jm1km1 = ( Tracer(i,j-1,km1) - Tracer(i,j-1,k) )
     &               * recip_drC(k)
        dR_W_km1    = 0.5d0 * ( maskFk(i,j-1) * dR_W_jm1km1 
     &               + maskFk(i,j) * dR_W_jkm1 )
        dR_W_jk     = ( Tracer(i, j ,k) - Tracer(i, j ,kp1) )
     &               * recip_drC(kp1)
        dR_W_jm1k   = ( Tracer(i,j-1,k) - Tracer(i,j-1,kp1) )
     &               * recip_drC(kp1)
        dR_W_k      = 0.5d0*( maskC(i,j-1,k ,bi,bj)
     &               * maskC(i,j-1,kp1,bi,bj)*maskp1*dR_W_jm1k
     &               + maskC(i,j,k ,bi,bj)*maskC(i,j,kp1,bi,bj)
     &               * maskp1*dR_W_jk)

        dCdz_S = 0.5d0 * ( dR_W_km1 + dR_W_k )
C-    Calculate the flux
        df(i,j)= df(i,j)-yA(i,j)*(Kyy_S*dCdy_S+Kyz_S*dCdz_S)
     &          *maskS(i,j,k,bi,bj)
       ENDDO
      ENDDO
