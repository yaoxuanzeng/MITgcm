...
C     slcon_Kyy/Kyz/Kzz :: Diffusivity tensor entries
      ...
      Real*8 slcon_lat(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, 1:Nr, nSx, nSy)
      Real*8 slcon_Hn(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, 1:Nr, nSx, nSy)
      Real*8 slcon_Kyy(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, 1:Nr, nSx, nSy)
      Real*8 slcon_Kyz(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, 1:Nr, nSx, nSy)
      Real*8 slcon_Kzz(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, 1:Nr, nSx, nSy)
...
C     sigmaY_C   :: Meridional density gradient at tracer grids
      ...
      Real*8 sigmaY_C(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8 sigmaR_C(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8 myTime
...
C     !LOCAL VARIABLES
      INTEGER i, j
      Real*8 d2r, clat, slat
      Real*8 bn, Kn, Kr, inv_f2
      Real*8 maskp1, maskloc, epsmask

      d2r = 3.141592653589793d0 / 180.d0
      epsmask = 1.D-20
      inv_f2 = 1.d0 / (2.d0*omega) / (2.d0*omega) 
...
C     Calculating density gradients at the tracer grids
      ...
      DO K = 1, Nr
       DO i = 1-OLx, sNx+OLx
        DO j = 1-OLy, sNy+OLy-1
         maskloc = maskS(i,j,K,bi,bj)
         maskp1 = maskS(i,j+1,K,bi,bj)
     ...
     &                    / (maskloc+maskp1+epsmask)
      ...
      DO j = 1-OLy, sNy+OLy
       DO i = 1-OLx, sNx+OLx
        sigmaR_C(i,j,1) = sigmaR(i,j,1)*maskW(i,j,1,bi,bj)
       END DO
      END DO
      ...
      DO K = 2, Nr
       DO j = 1-OLy, sNy+OLy
        DO i = 1-OLx, sNx+OLx
         maskloc = maskW(i,j,K,bi,bj)
         maskp1  = maskW(i,j,K-1,bi,bj)
         sigmaR_C(i,j,K) = (maskloc*sigmaR(i,j,K)
     &                    + maskp1*sigmaR(i,j,K-1))
     &                    / (maskloc+maskp1+epsmask)
        END DO
       END DO
      END DO
       ...
       DO j = 1-OLy, sNy+OLy
        DO i = 1-OLx, sNx+OLx
          ...
          ELSE
           ...
           bn = (clat*sigmaY_C(i,j,K)+slat*sigmaR_C(i,j,K))
...
C     If bn>0, it is stable
           IF (bn .GT. 0.d0) THEN
            ...
            slcon_Kzz(i,j,K,bi,bj) = 0.d0
           ELSE
            Kn = slcon_cn * ABS(bn) * SQRT(ABS(bn)) * inv_f2 
