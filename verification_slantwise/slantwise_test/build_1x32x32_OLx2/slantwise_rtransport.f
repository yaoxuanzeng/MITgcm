...
C     !LOCAL VARIABLES
      ...
      Real*8 epsW ! Avoiding zero denominator
      Real*8 dCdz_W, dCdy_W ! Tracer gradients at W grids
      Real*8 Kyz_W, Kzz_W ! Tensor elements at W grids
      Real*8 dY_S_jk, dY_S_jp1k, dY_S_jkm1, dY_S_jp1km1, dY_k, dY_km1 ! Gradients at the southern boundary, used to calculate dCdy_W 
...
C-    Calculate the tensor element at the W grids
        Kyz_W = (maskW(i,j,k  ,bi,bj)*slcon_Kyz(i,j,k  ,bi,bj)
     &          +maskW(i,j,km1,bi,bj)*slcon_Kyz(i,j,km1,bi,bj) ) 
     &          /(maskW(i,j,k  ,bi,bj)+maskW(i,j,km1,bi,bj)+epsW)
C     Adding Kzz in the slantwise_calc_diff.F
C        Kzz_W = (maskW(i,j,k  ,bi,bj)*slcon_Kzz(i,j,k  ,bi,bj)
C     &          +maskW(i,j,km1,bi,bj)*slcon_Kzz(i,j,km1,bi,bj) ) 
C     &          /(maskW(i,j,k  ,bi,bj)+maskW(i,j,km1,bi,bj)+epsW)

C-    Calculate the tracer gradients at the W grids
C     Calculate dCdz_W at the W grids 
C        dCdz_W =  _recip_drF(k) * ( Tracer(i,j,k) - Tracer(i,j,km1) ) * maskW(i,j,k,bi,bj)
C     Calculate dCdy_W at the W grids
     ...
     &                *recip_dyC(i,j+1,bi,bj)*recip_deepFacC(km1)
     &                *maskS(i,j+1,km1,bi,bj)
        dY_S_jkm1   = (Tracer(i,j  ,km1)-Tracer(i,j-1,km1))
     &                *recip_dyC(i,j  ,bi,bj)*recip_deepFacC(km1)
     &                *maskS(i,j  ,km1,bi,bj)
        dY_k   = 0.5d0 * ( dY_S_jp1k   + dY_S_jk   )
        dY_km1 = 0.5d0 * ( dY_S_jp1km1 + dY_S_jkm1 )
        dCdy_W = 0.5d0 * ( maskW(i,j,k  ,bi,bj) * dY_k
     &                 +   maskW(i,j,km1,bi,bj) * dY_km1 )

C-    Calculate the flux
C        df(i,j) = df(i,j)-rA(i,j,bi,bj)*(Kyz_W*dCdy_W+Kzz_W*dCdz_W)
C     &           *maskW(i,j,k,bi,bj)*maskFk(i,j)
         df(i,j) = df(i,j)-rA(i,j,bi,bj)*(Kyz_W*dCdy_W)
     &            *maskW(i,j,k,bi,bj)*maskFk(i,j)
